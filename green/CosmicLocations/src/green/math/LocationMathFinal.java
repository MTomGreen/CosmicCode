...
//		Tc = 400E-9;
	...
	public static VectorDouble getAnglesForCoincidenceWithLog(Coincidence con, ArrayList<String> log){
		...
		//Validate UNIX time
		if(!(eventA.unixTime == eventB.unixTime && eventB.unixTime == eventC.unixTime)){ 
			log.add("The unix timestamps were different. For complexity reasons, and due to the rarity of this happening, the function will exit.");
			System.err.println("returned 0 and 0 degrees");
			return null;
		}
		
		log.add(eventA.rawData);
		log.add(eventB.rawData);
		log.add(eventC.rawData);
		...
		log.add(detectorA.getName() + " (" + detectorA.getStationID() + ")");
		log.add(detectorB.getName() + " (" + detectorB.getStationID() + ")");
		log.add(detectorC.getName() + " (" + detectorC.getStationID() + ")");
...
//		Tc = 400E-9;
	    ...
	    log.add("Azimuth: " + azimuth);
	    log.add("Zenith: " + zenith);
